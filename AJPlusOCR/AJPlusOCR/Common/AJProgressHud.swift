...
class AJProgressHud: NSObject {
    ...
    //显示信息
    class func showMessageView(message: String, view: UIView?) -> MBProgressHUD {
        var view = view
        if (view == nil) {
            view = UIApplication.shared.keyWindow
        }
    ...
    @discardableResult class func showMessage(_ message: String = "") -> MBProgressHUD {
        return showMessageView(message: message, view: nil)
    }
